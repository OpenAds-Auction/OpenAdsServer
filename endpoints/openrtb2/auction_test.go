...
"id": "some-request-id",
	...
	endpoint, _ := NewEndpoint(
		...
		analyticsBuild.New(&config.Analytics{}, &metricsConfig.NilMetricsEngine{}),
		map[string]string{},
		[]byte{},
...
func TestValidateRequestExt(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedErrors:  []string{`request.ext is invalid: request.ext.prebid.cache requires one of the "bids", "vastxml", or "collatevast" properties`},
		},
		{
