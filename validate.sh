...
if $COVERAGE; then
  ...
  go test -timeout 120s $(go list ./... | grep -v /vendor/ | grep -v /adapters/; go list ./adapters/thetradedesk/...)
fi
...
if [ "$RACE" -ne "0" ]; then
  go test -race $(go list ./... | grep -v /vendor/ | grep -v /adapters/; go list ./adapters/thetradedesk/...) -run ^TestRace.*$ -count $RACE
fi
